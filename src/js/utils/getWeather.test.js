...
describe('Get weather data', () => {
	...
	it('Get weather data by city name', async () => {
		...
		expect(fetch.mock.calls[0][0]).toEqual(`https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&appid=${OPENWEATHERMAP_API_KEY}`);
	});
	...
	it('Get weather data by coordinates', async () => {
		...
		expect(weatherByCity.name).toEqual(city);
		expect(fetch.mock.calls.length).toEqual(1);
		expect(fetch.mock.calls[0][0]).toEqual(`https://api.openweathermap.org/data/2.5/weather?lat=${lt}&lon=${ln}&units=metric&appid=${OPENWEATHERMAP_API_KEY}`);
	});
